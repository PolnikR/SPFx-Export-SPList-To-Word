...
import Export2WordCommandSet from './Export2WordCommandSet';
...
class Convert2Doc {
    ...
    public generateTableFormat(items: IListItemsResponse): void {
        ...
        items.result.forEach(i => {
            ...
            this.listFieldDetails.forEach(k => {
                ...
                this.cisloZiadanky = `${new Date().getFullYear()}/${k.InternalName["ID"].toString()}`;
    ...
    * @param sourceHTML 
    */
    public generateDocument(sourceHTML: string, cisloZiadanky:string): void {
